...
> 目标域名：`drop.28207.cc`  
> 核验日期：2026-08-12  
> **当前状态（2026-08-14 更新）**：本方案是原始实施蓝图，按阶段落地时的
> 实际状态请以 `docs/roadmap.md` 为准。已发生的变化：Phase 00–09 全部实施；
> Phase 10（Tauri）已取消（D8）；Phase 11（E2EE）已实施后整体移除（D13）；
> Phase 07 分享密码、Phase 08 收件上传、预览白名单已按 D14 移除（对应列与
> migration 保留惰性）。下文仍保留原始设计供参考，不代表当前实现。
